...
class TestNode:
    ...
    def test_is_archived_returns_true_if_status_property_is_archived(self, create_node):
        node = create_node(status=ItemStatus.ARCHIVED)

        assert node.is_archived is True

    def test_is_archived_returns_false_if_status_property_is_not_archived(self, create_node):
        node = create_node(status=ItemStatus.ACTIVE)
